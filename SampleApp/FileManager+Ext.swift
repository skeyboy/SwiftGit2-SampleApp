...
extension URL {
    var repositoryHome : URL {
        let dest = [FileManager.DocumnetsDirectory(), "Repository" ].joined(separator: "/")
        if self.absoluteString == dest {
            return self
        } else {
            return URL(fileURLWithPath: dest)
        }
//        if self.absoluteString.split(separator: FileManager.DocumnetsDirectory()).last == "Repository" {
//            return self
//        } else {
//            return self.appendingPathComponent("Repository")
//        }
    }
}
...
public extension FileManager {
    ...
    static let repository : String = ["Repository", DocumnetsDirectory()].joined(separator: "/")
